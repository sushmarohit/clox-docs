import type { HomeCopy } from '@/components/home/copy';
import { featureImages } from '@/components/home/types';

export function HomeFeatures({ copy }: { copy: HomeCopy }) {
  return (
    <section id="features" className="scroll-mt-[9.5rem] bg-slate-950 py-14 text-white sm:scroll-mt-28 sm:py-28">
      <div className="clox-container">
        <h2 className="mb-4 text-center text-[1.65rem] font-extrabold uppercase leading-tight text-white sm:text-[2.8rem] 3xl:text-[3.2rem]">
          {copy.featuresTitle}
        </h2>
        <div className="mt-8 grid gap-6 sm:mt-16 sm:gap-8 lg:grid-cols-3 lg:grid-rows-2 3xl:gap-10">
          {copy.features.map(([title, body], index) => {
            const isVerification = index === 2;

            return (
              <article
                key={title}
                className={`group relative min-h-[320px] overflow-hidden rounded-3xl border border-white/5 bg-slate-800 transition duration-500 hover:-translate-y-2 hover:border-[rgba(255,86,14,0.4)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)] 3xl:min-h-[380px] ${
                  isVerification ? 'lg:row-span-2' : ''
                }`}
              >
                <div
                  className="absolute inset-0 bg-cover bg-center opacity-50 transition duration-500 group-hover:scale-105 group-hover:opacity-80"
                  style={{ backgroundImage: `url('${featureImages[index]}')` }}
                  aria-hidden
                />
                <div
                  className={`absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-slate-950 via-slate-950/70 to-transparent p-8 ${
                    isVerification ? 'pt-20' : 'pt-24'
                  }`}
                >
                  <h3 className="mb-2 text-[1.4rem] font-extrabold text-clox-orange sm:text-[1.8rem]">
                    {title}
                  </h3>
                  {isVerification ? (
                    <ul className="mt-3 space-y-2.5">
                      {copy.featureVerificationItems.map((item) => (
                        <li
                          key={item}
                          className="flex items-start gap-2.5 text-[0.95rem] leading-6 text-slate-200 sm:text-[1.05rem] sm:leading-7"
                        >
                          <span className="mt-0.5 shrink-0 font-black text-clox-orange" aria-hidden>
                            ✓
                          </span>
                          <span>{item}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-[1.1rem] leading-7 text-slate-200">{body}</p>
                  )}
                </div>
              </article>
            );
          })}
        </div>
      </div>
    </section>
  );
}
